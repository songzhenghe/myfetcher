...
class inst extends super_inst{
	...
	public function mp3(){
		$this->mymkdir("E:/wwwroot/myfetcher/callback/frword/mp3/");
	}
	...
	public function doit($info){
		...
		$url="http://www.frdic.com/dicts/fr/".rawurlencode($info['word']);
		$c=$this->request(0,$url,$this->cookie_jar,'http://www.frdic.com/');
		...
		$c2=preg_match_all('/<input type="hidden" id="page-status" value="(.*?)" lang="fr" word=".*?">/ims',$c,$matches);
		if($c2){
			$key=$matches[1][0];
			
			
			if($key){
				$c8=$this->request(1,'http://www.frdic.com/Dicts/fr/tab-detail/-12',$this->cookie_jar,'http://www.frdic.com/',array('status'=>$key));
				...
				if($c2){
					...
					$query="update `frword` set `sentence`='{$c2}' where `id`='{$info['id']}'";
